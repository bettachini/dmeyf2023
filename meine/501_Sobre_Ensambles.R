...
"Muchos modelos, que se combinan para dar una predicción."

## - ¿Cómo tienen que ser los modelos dentro de un ensamble?
"Distintos algoritmos, no basta diversos valores de hiperparámetros.
- árboles
- regresión logística
- SVM: lineal, radial, polinómico
- KNN
Encuentran distntos patrones en los datos. Discriminan distintas distribuciones morfológicas en el espacio de variables.
"
## - ¿Qué técnicas conoce para ensamblar modelos?
"
Combinarlos
- votos (clase)
- promedio score/probabilidad

Estratégias de construcción:
- Bagging: variedad la buscamos con datos al azar (bootstrapping)
      - Boostrapping: nos quedamos con un % de casos y features elegidos al azar, agrega el complemento con datos al azar con o sin reemplazo
- Boosting
"
...
print(
  sum(
    (pred_train$predictions[, "evento"] >= 0.025) *
      ifelse(
...
print(
  sum(
    (pred_test$predictions[, "evento"] >= 0.025) *
      ifelse(
...
importancia <- as.data.table(
  modelo_rf_1$variable.importance,
  keep.rownames = TRUE
)
...
"No ver train vs test. Lo importante es la curva de test, no ver la diferencia (esto es para ensambles)"
...
importancia2 <- as.data.table(
  modelo_rf_2$variable.importance,
  keep.rownames = TRUE
)
...
"Los modelos deben ser profundos. Que tengan a overfittear no es un problema."
...
importancia3 <- as.data.table(
  modelo_rf_3$variable.importance,
  keep.rownames = TRUE
)
